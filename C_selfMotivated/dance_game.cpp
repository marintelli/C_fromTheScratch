...
//Cursor move 커서를 특정 위치로 이동시키기 위한 함수
void gotoxy(int x, int y)
{
    COORD Pos;  //x, y 위치를 가지고 있는 구조체
    Pos.X = 2 * x;  //커서가 2씩 이동하는게 더 자연스러워 보이기 때문에 2*x를 넣음
    Pos.Y = y;
    //SetConsoleCursorPosition콘솔의 커서 위치 알려주는 함수, 매개변수GetStdHandle(STD_OUTPUT_HANDLE)화면출력 핸들러, 매개변수 COORD타입 변수Pos 
...
void StartGame()
{
    /*const char* c = "HYP-Hit.wav";
    LPCWSTR str = TEXT("HYP-Hit.wav");
    ...
    char buffer[] = "HYP-Hit.wav";
    wchar_t text1[30];
    mbstowcs(text1, buffer, strlen(buffer) + 1);
    LPCWSTR test = text1;*/

    PlaySound(TEXT("HYP-Hit.wav"), NULL, SND_NODEFAULT | SND_ASYNC | SND_LOOP);
    int life = LIFE;
    int score = 0;
    //재생했을때 현재시간.
